...
Real VL_Algorithm_3D_CUDA(Real *host_conserved0, Real *host_conserved1, int nx, int ny, int nz, int x_off, int y_off, int z_off, int n_ghost, Real dx, Real dy, Real dz, Real xbound, Real ybound, Real zbound, Real dt, int n_fields, Real t)
{
  ...
  if ( !memory_allocated ){
    ...
    CudaSafeCall( cudaMalloc((void**)&dev_flux_array, 2*3*nx*ny*nz*sizeof(Real)) );
    #endif
  ...
  while (block < block_tot) {
    ...
    // Apply cooling
    #ifdef COOLING_GPU
    cooling_kernel<<<dim1dGrid,dim1dBlock>>>(dev_conserved, nx_s, ny_s, nz_s, n_ghost, n_fields, dt, gama, dev_dt_array, t);  
    CudaCheckError();
    #endif
    ...
    apply_heat_fluxes_kernel<<<dim1dGrid, dim1dBlock>>>(dev_conserved, dev_flux_array, nx_s, ny_s, nz_s, n_ghost, dt, dx, dy, dz, dev_dt_array);
